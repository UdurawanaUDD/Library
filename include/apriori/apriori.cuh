...
class tuple {
    ...
    void printResult(){
        vector<int>::iterator re = values.begin();
        while(re != values.end()) {
            cout<<*re<<" ";
            re++;                                 
        }
...
//#ifdef TEST_PARAMS
   ...
   // #endif
    ...
    stopTime(&timer);// cout<<elapsedTime(timer)<<endl;
    totalRunTime += elapsedTime(timer);
    cout<<"-----C1 Frequent Itemsets----"<<endl;
    for (int i = 0; i < MAX_UNIQUE_ITEMS; i++) {
        if(ci_h[i]!= 0){
        cout<<i<<"---(frequency)----->>  "<<ci_h[i]<<endl;
        }    
    }   
    startTime(&timer);
   ...
   // }
    ...
    cout<<"----C2 Frequent itemsets ----"<<endl;
    for(int i = 0; i< maskLength; i++) {
        m = i/k;
        n = i%k;
        if(mask_h[i]!=0){
        cout<<li_h[m]<<" "<<li_h[n]<<"---(frequency)----->> "<<mask_h[i]<<endl;
        }
    }
    if(maskLength<2){
       return 3;
    ...
    totalRunTime += elapsedTime(timer);
#ifdef TEST_PARAMS
    cout<<"############################## mask_h after join Debug Using ################"<<endl;
    for (int i = 0;i < k; i++) {
        for (int j = 0;j < k;j++) {
            cout<<mask1_h[i*k + j]<<" ";
        }
    ...
    unsigned int *b;
    unsigned int *c;
    num1 = (unsigned int*) malloc(patterns1.size()*sizeof(unsigned int));
    num2 = (unsigned int*) malloc(patterns1.size()*sizeof(unsigned int));
    a = (unsigned int*) malloc(patterns1.size()*sizeof(unsigned int));
    b = (unsigned int*) malloc(patterns1.size()*sizeof(unsigned int));
    c = (unsigned int*) malloc(patterns1.size()*sizeof(unsigned int));

    for (int i = 0; i<sparse_matrix_size1;i++) {
        num1[i] = new_new_patterns[sparseM_h1[i]];
        num2[i] = new_new_patterns[sparseM_h1[i + sparse_matrix_size1]];
        a[i] = num1[i]/mul_factor;
        b[i] = num1[i]%mul_factor;
        if((a[i] != num2[i]/mul_factor)&&(b[i] != num2[i]/mul_factor)){
            c[i] = num2[i]/mul_factor;
        }
        else
            c[i] = num2[i]%mul_factor;
    }
    ...
    r_a = (unsigned int*) malloc(patterns1.size()*sizeof(unsigned int));
    r_b = (unsigned int*) malloc(patterns1.size()*sizeof(unsigned int));
    r_c = (unsigned int*) malloc(patterns1.size()*sizeof(unsigned int));
    vector<std::pair<vector<int>, int> > results;
    for(int i = 0; i<sparse_matrix_size1; i++) {
        for (it_modulo_map = new_modulo_map.begin(); it_modulo_map != new_modulo_map.end();it_modulo_map++){
            if(it_modulo_map->second == a[i])
                r_a[i] = it_modulo_map->first.get(0);
            if(it_modulo_map->second == b[i])
                r_b[i] = it_modulo_map->first.get(0);
            if(it_modulo_map->second == c[i])
                r_c[i] = it_modulo_map->first.get(0);
        }
    vector<int> v_tuple;
    v_tuple.push_back(r_a[i]);
    v_tuple.push_back(r_b[i]);
    v_tuple.push_back(r_c[i]);
    std::sort(v_tuple.begin(),v_tuple.end());
    int item1 = sparseM_h1[i + 2 * sparse_matrix_size1];
    results.push_back(std::pair<vector<int> ,unsigned int>(v_tuple,item1));
    }
    cout<<"~~~~~~~~~~~~~~~~~~~~~~ C3 Frequent itemsets ~~~~~~~~~~~~~~~~~~~~"<<endl;
    bool mv=true;
    int index=4;
    while(true) {
       if(mv){
          vector<std::pair<vector<int> , int> >::iterator it_res;
          for (it_res = results.begin(); it_res != results.end();it_res++) {
             L[it_res->first] = it_res->second;
          }
          output(L);
          mv=!mv;
       }
